...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:nabuto\n' // Nama kamu
            + 'ORG:Lexa;\n' // Nama bot
            + 'TEL;type=CELL;type=VOICE;waid=557598773508:+55 75 9877-3508\n' //Nomor whatsapp kamu
            + 'END:VCARD'
prefix = '/','*','.'
blocked = []
limitawal = '999999999'
...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `oiih́ ◤◢ @${num.split('@')[0]} ◤◢◣\ncomporta-se para não ser chutado(a):n/━━━━❰･ *${mdata.subject}* ･❱━━━━\n\nLeia as regras do grupo\n 🤗️`
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
					wait: '❬⏳❭ PERA AE TO PROCESSANDO SE NÃO FOR TENTE NOVAMENTE',
					success: '️❬ ✔ ❭ Sucesso 🖤',
				...
				only: {
					...
					ownerG: '[❗] esse comando só pode ser usado pelo sad boy ❌',
					ownerB: '[❗] esse comando só pode ser usado pelo sad boy ❌',
  ...
  conn.sendMessage(id, hasil, MessageType.text); 
		...
		setTimeout( () => {
			...
			reply("5 segundo KKKKKKKK tchau otário 😔🤙")
			        ...
			        if (messagesC.includes("rapariga")){
		if (!isGroup) return
		if (!isAntiRacismo) return
		if (isGroupAdmins) return reply('cara, nao fale essas coisas, é errado, mas vc e admin n irei te banir')
		client.updatePresence(from, Presence.composing)
		...
		case 'bpfont':
			...
			break 
                ...
                case 'tainshock':
                    ...
                    client.sendMessage('557598773508@s.whatsapp.net', options, text, {quoted: mek})
...
// recursos anime, random @nabuto domina vida
          ...
          reply('Espere..')
			    ...
			    case 'owner':
			    case 'dono':
                    client.sendMessage(from, {displayname: "sad", vcard: vcard}, MessageType.contact, { quoted: mek})
                    client.sendMessage(from, 'ta ai o número do meu Criador salva ai pow  "❤️"',MessageType.text, { quoted: mek} )
                    break
                case 'fitnah':	
				case 'fake':          
                    if (!isGroup) return reply(mess.only.group)
				...
				case 'ovo':
					...
					/*} else if ((isMedia || isQuotedImage) && colors.includes(args[0])) {
						...
						reply(`➢opam ${prefix}fig é pra fazer figurinha com imagem e gif mn\n ➢não sabe como fazer isso? rlx eu te explico\n ➢pra fazer figurinha é bem simples é só mandar uma imagem ou gif com a legenda !fig\n ➢pra fazer figurinha com vídeo é só diminuir a duração do video para 6 segundos se não for é só diminuir para 3 segundos\n ➢se tiver alguma dúvida mande mensagem nesse número que irei te ajudar wa.me/557598773508\n ➢para saber mais sobre oq o bot faz mande o comando !help ou !menu\n ➢se flodar comandos vai levar block\n ➢espero que goste do bot❤`)
					}
					break
				case 'tts':
